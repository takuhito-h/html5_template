...
| スタイル | Sass（コンポーネントスコープ+ Tailwind CSS v4  |
...
### CSS
- 基本的なスタイルは各 `.astro` ファイルの `<style lang="scss">` タグに記述する
- レスポンシブ対応には  `@media` / `@container` を使う
- ユーティリティにはtailwindの余白追加のクラスを使用する
...
<script>
    // DOM 操作・イベント処理はここに記述する
    // 例: document.querySelectorAll<HTMLElement>('.js-xxx').forEach(...)
</script>

<style lang="scss">
    .component-name {
        // コンポーネント固有のスタイルはここに記述する
    }
</style>
```

### 使い分けのルール
- `<script>` — DOM 操作・イベント処理（TypeScript 使用可）
- `<style lang="scss">` — コンポーネント固有スタイル（自動スコープ）
- `<style is:global lang="scss">` — `body` など親要素を参照するスタイルのみ
- Props のオプショナルな値には `?` と初期値（`= ''` など）を必ず付ける
