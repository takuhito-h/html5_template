...
- `pnpm build`：Astro ビルド（`dist/` 出力）と JPG/PNG の WebP 変換をまとめて実行します。
- `pnpm build:astro`：Astro のみでビルドする際に利用します。
- `pnpm build:webp`：`sharp-cli` を使って `public/img/` 内の画像を WebP 化します。
